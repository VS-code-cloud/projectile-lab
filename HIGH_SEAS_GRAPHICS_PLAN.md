...
- **2026-06-28 — Phase 1 underway (verified):**
  ...
  - ⏸️ **P1-T2 Environment/IBL — deferred**: the custom water shader does its own sun specular + Fresnel, so an env map adds little now; revisit if PBR ship/asset reflections are wanted (would also feed Phase 2 GLTF assets). (Also: drei presets pull an HDRI asset over the network, which the "no models/assets" scope excludes.)
  - ✅ **P1-T4 wake** — `src/highseas/components/SeaWake.tsx`: a speed-driven feathered foam ribbon trailing astern (additive, depth-write off, `toneMapped={false}` so Bloom catches it), mounted inside the heading-rotated player-ship group so it always trails the bow. Length/opacity scale with sail speed read from a `sim.speed` ref (no React state); churns via `uTime`; off under reduced motion. Pure `src/highseas/wakeField.ts` (`wakeIntensity`/`wakeLength`/`wakeOpacity`) + `src/test/highSeasWakeField.test.ts` (5 tests).
  - ✅ **Phase 1 complete** (the no-model "preliminary" atmosphere layer). ⬜ **Remaining**: Phase 2 (terrain/assets/collision/shadows — *needs* GLTF/Blender or carries shader-terrain + water-shadow-receive risk; out of the current no-model scope); Phase 3 (perf/LOD/robustness).
  - Status: `npm run lint` clean, `build` passing, **224 tests** green.

- **2026-06-28 — Phase 2 P2-T2 (partial) · env-gated HD asset layer + full E2E verification:**
  - ✅ **`new_assets_enabled` contract** — `src/highseas/assets.ts` exposes `newAssetsEnabled()` (reads `VITE_NEW_ASSETS_ENABLED`, plus a `?newAssets=1` dev URL override) and the HD asset URL registry. Documented in `.env.example`; unit-tested in `src/test/highSeasAssets.test.ts`. The flag is computed once in `OpenSeaScene3D` and threaded down so **both** the old procedural path and the new GLTF/HDRI path render the same UI.
  - ✅ **CC0 assets sourced online** — GLB models (`ship`, `palm`, `rock` from Poly Pizza / Quaternius) + a 1k HDRI (`sky_1k.hdr` from Poly Haven) downloaded into `public/assets-hd/{models,hdri}` with attributions in `public/assets-hd/CREDITS.txt`.
  - ✅ **Asset render components** — `AssetShip.tsx` (GLTF ship w/ clone + per-instance tint + heading offset), `AssetIslandDecor.tsx` (palms/rocks on the plateau), `AssetSky.tsx` (HDRI environment + IBL), all wrapped in `AssetErrorBoundary.tsx` + `Suspense` so a missing/failed asset silently falls back to the procedural mesh. Integrated into `OpenSeaScene3D` via `ShipModel`/`HdAssets` helpers (player + contact ships, town island). Browser-verified both paths render (procedural default; `?newAssets=1` swaps in models + HDRI sky).
  - ✅ **Survivable-encounter fix** — pure `wouldSink(save, result)` in `voyage.ts` mirrors `applyResult`'s 1-HP floor for `survivable` encounters; `HighSeasPage.handleResolve` now uses it instead of a raw `hullHp - damage <= 0` check, so a failed whirlpool / navy surrender no longer triggers a false game-over. Covered in `highSeasVoyage.test.ts` + `highSeasEncounterFlow.test.ts`.
  - ✅ **End-to-end tests (code)** — `src/test/highSeasEncounterFlow.test.ts`: winnability across 201 seeds for all 6 challenges (cannon, jettison, overboard stop, whirlpool, kedge, heel), success resolutions via `applyResult`, and fail/`survivable` rules incl. `wouldSink` parity with `applyResult`.
  - ✅ **End-to-end tests (browser, screenshot-driven)** — all 6 mini-game harnesses (`/dev/cannon`, `/dev/moor-stop`, `/dev/jettison`, `/dev/kedge`, `/dev/maelstrom`, `/dev/heel-deck`) exercised for trigger + success, plus an explicit fail state on the overboard rescue (under-judged glide). Full-game flow verified in `/dev/high-seas?newAssets=1`: open sea → navy pursuit → resolve → moor at Tortuga (kedge success) → dock → enter town/market. The previously-reported whirlpool bug (invisible vortex / unclear answer) is fixed — the spiral is visible throughout and the orbit reads `a = v²/r`.
  - ✅ **Vercel dep fix** — root `.npmrc` `legacy-peer-deps=true` (not gitignored) resolves the `postprocessing@6.39.1` vs `three@0.185` ERESOLVE; fresh `npm install` audits clean.
  - Status: `npm run lint` clean, `build` passing, **251 tests** green.

- **2026-06-28 — Gameplay pass alongside Phase 1 finish (not graphics, logged for context):** persistent approaching enemy contacts (no 15 s despawn; chase the player; leave only on fight/moor; trickle-spawn up to a cap), boat position persisted on a navy escape (no snap-back to port), and gentler sail accel/drag (same top speed, ~6 s to wind up). Lives in `navigation.ts` (`approachPosition`, lower `SAIL_ACCEL`/`SAIL_DRAG`), `worldEncounters.ts` (`spawnContact`, `contactChaseSpeed`, chase/cap consts), `HighSeasPage.tsx` (live `contactsRef` + throttled `contactDots`), `OpenSeaScene3D.tsx` (reads live contacts each frame, ships turn to bear on the player). Upholds rendering invariant #1: `contacts` membership (→ memoized canvas) changes only on spawn/fight/moor; positions ride refs + a 10 Hz `contactDots` snapshot.
